  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    if (!isset($entity->path->pathauto)) {
      if (!$entity->isNew()) {
        ...
        $path = \Drupal::service('path.alias_manager')->getAliasByPath($entity->urlInfo()->toString(), $entity->language()->getId());
        $pathauto_alias = \Drupal::service('pathauto.manager')->createAlias($entity->getEntityTypeId(), 'return', $entity->urlInfo()->toString(), array($entity->getEntityType()->id() => $entity), $entity->bundle(), $entity->language()->getId());
        $entity->path->pathauto = ($path != $entity->urlInfo()->toString() && $path == $pathauto_alias);
      }
      else {
